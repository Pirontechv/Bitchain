...
#if defined(NDEBUG)
#error "BITCHAIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uxbit) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, int nHeight, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zXBIT spend with serial %s is already in block %d\n", __func__,
...
{
<<<<<<< HEAD
    //Check to see if the zXBIT is properly signed
...
>>>>>>> 5bd387e18d4a354a6821b80f48b0b9fdf0a18bc8
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zXBIT spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zXBIT without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
...
<<<<<<< HEAD
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zXBIT spend with serial %s from tx %s is not in valid range\n", __func__,
=======
        if (!isBlockBetweenFakeSerialAttackRange(nHeight))
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZXBITModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZXBITModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zXBIT spend tx %s already in block %d",
...
<<<<<<< HEAD
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxbit");
                }
...
<<<<<<< HEAD
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZXBITModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
                    ...
                    if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                        return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                                __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zxbit");
                    }
...
<<<<<<< HEAD
            // Check that zXBIT mints (if included) are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZXBITStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zXBIT is staked, masternode only gets 2 XBIT
        ret = 3 * COIN;
        if (isZXBITStake)
            ret = 2 * COIN;
    }
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZXBITModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BITCHAIN
...
{
    RenameThread("bitchain-scriptch");
...
void RecalculateZXBITMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
<<<<<<< HEAD
    uiInterface.ShowProgress(_("Recalculating minted ZXBIT..."), 0);
=======
    const int chainHeight = chainActive.Height();
...
<<<<<<< HEAD
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZXBIT..."), percent);
=======
...
void RecalculateZXBITSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
<<<<<<< HEAD
    uiInterface.ShowProgress(_("Recalculating spent ZXBIT..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZXBIT..."), percent);
=======
    const int chainHeight = chainActive.Height();
...
>>>>>>> 5bd387e18d4a354a6821b80f48b0b9fdf0a18bc8
        ...
        //Remove spends from zXBIT supply
...
bool RecalculateXBITSupply(int nHeightStart)
{
    const int chainHeight = chainActive.Height();
    ...
    uiInterface.ShowProgress(_("Recalculating XBIT supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
<<<<<<< HEAD
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating XBIT supply..."), percent);
=======
...
bool UpdateZXBITSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zXBIT supply was off (due to serial duplication off-chain on main net)
...
<<<<<<< HEAD
            // Check that zXBIT mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
>>>>>>> 5bd387e18d4a354a6821b80f48b0b9fdf0a18bc8
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZXBITMinted();
        RecalculateZXBITSpent();
        RecalculateXBITSupply(Params().Zerocoin_StartHeight());
    }

    //Track zXBIT money supply in the block index
    if (!UpdateZXBITSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zXBIT supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 XBIT.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXBIT spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXBIT serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Bitchain Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().BitchainBadBlockTime()) && (block.nBits == (uint32_t) Params().BitchainBadBlocknBits())) {
            // accept BITCHAIN block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> xbitInputs;
        std::vector<CTxIn> zXBITInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zXBITInputs.push_back(stakeIn);
            }else{
                xbitInputs.push_back(stakeIn);
            }
        }
        const bool hasXBITInputs = !xbitInputs.empty();
        const bool hasZXBITInputs = !zXBITInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasXBITInputs) {
                    ...
                    for (const CTxIn& xbitIn : xbitInputs)
                        if(xbitIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zXBIT inputs.
            if(hasZXBITInputs) {
                for (const CTxIn& zXBITInput : zXBITInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zXBITInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxbit");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BITCHAIN: We use certain sporks during IBD, so check to see if they are
