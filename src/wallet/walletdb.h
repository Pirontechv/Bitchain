...
public:
    ...
    bool WriteZXBITSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZXBITSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZXBITSeed_deprecated(uint256& seed);
    bool EraseZXBITSeed();
    bool EraseZXBITSeed_deprecated();

    bool WriteZXBITCount(const uint32_t& nCount);
    bool ReadZXBITCount(uint32_t& nCount);
