...
#include "zxbit/zxbitmodule.h"
#include "zxbit/zxbitwallet.h"
#include "zxbit/zxbittracker.h"
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 XBIT at the same time
    ONLY_10000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for zXBIT send
enum ZerocoinSpendStatus {
    ZXBIT_SPEND_OKAY = 0,                            // No error
    ZXBIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXBIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXBIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXBIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXBIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXBIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXBIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXBIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXBIT_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZXBIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXBIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXBIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXBIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXBIT_SPENT_USED_ZXBIT = 14,                      // Coin has already been spend
    ZXBIT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXBIT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 XBIT output and keys which can be used for the Masternode
    ...
    bool CreateZXBITOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zXBIT wallet
    CzXBITWallet* zwalletMain;
    std::unique_ptr<CzXBITTracker> zxbitTracker;
    void setZWallet(CzXBITWallet* zwallet);
    CzXBITWallet* getZWallet();
    ...
    // zXBIT reset
    boost::signals2::signal<void()> NotifyzXBITReset;
};
