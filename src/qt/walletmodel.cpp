...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 XBIT.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZxbitSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZxbit(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt
){
...
static void NotifyzXBITReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXBITReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXBITReset.connect(boost::bind(NotifyzXBITReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXBITReset.disconnect(boost::bind(NotifyzXBITReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zxbitTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
