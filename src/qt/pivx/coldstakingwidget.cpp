...
#include "qt/bitchain/forms/ui_coldstakingwidget.h"
...
ColdStakingWidget::ColdStakingWidget(BITCHAINGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your XBITs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept XBIT delegation / Delegate XBIT"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select XBIT outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 XBIT" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
