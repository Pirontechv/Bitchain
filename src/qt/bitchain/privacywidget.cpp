    ...
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zXBIT anonymizes your XBIT by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zXBIT or convert back to XBIT"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 XBIT ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zXBIT");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zXBIT");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zXBIT");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zXBIT");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zXBIT");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zXBIT");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zXBIT");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zXBIT");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zXBIT Movements"));
    ...
    // Only Convert to XBIT enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzXBIT->setTitleClassAndText("btn-title-grey", "Total 0 zXBIT");
    ui->btnTotalzXBIT->setSubTitleClassAndText("text-subtitle", "Show denominations of zXBIT owned.");
    ui->btnTotalzXBIT->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select XBIT outputs to mint into zXBIT.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zXBIT");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzXBIT, SIGNAL(clicked()), this, SLOT(onTotalZxbitClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zXBIT");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of XBIT to mint into zXBIT"));
    }else{
        btnText = tr("Convert back to XBIT");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zXBIT to convert back into XBIT"));
    }
...
void PrivacyWidget::onTotalZxbitClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzXBIT->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzXBIT->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zXBIT is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zXBIT, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zXBIT").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zXBIT minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZxbit(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("zXBIT converted back to XBIT"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXBIT </b>";
    ...
    ui->btnTotalzXBIT->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
