...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::XBIT);
    ...
    if (settings.contains("nAnonymizeBitchainAmount") || reset)
        SoftSetArg("-anonymizebitchainamount", settings.value("nAnonymizeBitchainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitchain.cpp during shotdown to also remove the window positions
