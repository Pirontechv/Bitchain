...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter BITCHAIN address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no BITCHAIN: URI
...
{
    // Convert bitchain:// to bitchain:
    //
    //    Cannot handle this later, because bitchain:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open bitchain.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BITCHAIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BITCHAIN.lnk
...
{
    return GetAutostartDir() / "bitchain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitchain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BITCHAIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitchain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitchain app to startup item list
