...
CzXBITWallet::CzXBITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxbit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXBITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXBITSeed_deprecated()) {
                    LogPrintf("%s: Updated zXBIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxbit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxbit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXBITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXBITCount(nCountLastUsed);
    else if (!walletdb.ReadZXBITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXBITWallet::Lock()
{
    seedMaster = 0;
}

void CzXBITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXBITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXBIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXBITWallet::LoadMintPoolFromDB()
{
...
void CzXBITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXBITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXBITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxbitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXBITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXBIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxbitTracker which also adds to database
    pwalletMain->zxbitTracker->Add(dMint, true);
...
void CzXBITWallet::SeedToZXBIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXBITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXBITWallet::GenerateDeterministicZXBIT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXBITWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXBITWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXBITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
