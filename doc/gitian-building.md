...
*Setup instructions for a gitian build of BITCHAIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BITCHAIN
...
to the BITCHAIN GitHub release page.
...
- [Building BITCHAIN Core](#building-bitchain-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitchain-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitchain-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitchain-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitchain-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitchain-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitchain-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/Pirontechv/Bitchain/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bitchainproject.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitchain-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitchain-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitchain-osx-*-build.assert
```
